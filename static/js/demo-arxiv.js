...
console.log = function(...args) {
    ...
    if (msg.includes('[DEBUG]')) {
        window.__debug_logs__.push(msg);
    }
};
...
function getTotalFrames() {
    return (objectPoses && objectPoses.length > 0) ? objectPoses.length : (currentMetadata ? currentMetadata.num_frames : 0);
}
...
const FrameLoadQueue = {
    ...
    process(parentDir, loadToken) {
        ...
        const end = Math.min(totalFrames - 1, currentFrameIndex + WINDOW_NEXT);
...
function setupControlPanelEvents() {
    ...
    if (frameIndexSlider && frameIndexValue) {
        ...
        frameIndexSlider.addEventListener('input', (e) => {
            const totalFrames = getTotalFrames();
            if (!isAnimationPaused || totalFrames === 0) return;
    ...
    if (prevFrameButton) {
        prevFrameButton.addEventListener('click', () => {
            ...
            updateFrame();
            updateDynamicWindow(currentFrameIndex);
            frameIndexSlider.value = (currentFrameIndex / (totalFrames - 1)) * 100;
            frameIndexValue.textContent = `${currentFrameIndex + 1}/${totalFrames}`;
        });
    }
    ...
    if (firstFrameButton) {
        firstFrameButton.addEventListener('click', () => {
            ...
            frameIndexValue.textContent = `1/${totalFrames}`;
        });
    }
    ...
    if (lastFrameButton) {
        lastFrameButton.addEventListener('click', () => {
            ...
            frameIndexValue.textContent = `${totalFrames}/${totalFrames}`;
        });
    }
    ...
    if (resetViewBtn) {
        resetViewBtn.addEventListener('click', () => {
            ...
            const targetCenter = handCentroid ? handCentroid.clone() : new THREE.Vector3(0, 0, initObjectPosition.z);
            ...
            setTimeout(() => {
                ...
                if (handCentroid) {
                    controls.target.copy(handCentroid);
                } else {
...
function updateFrame() {
    if (handMeshes.length === 0 || videoFrames.length === 0 || cameraFrustums.length === 0) {
        return;
    }

    // 更新物体位置和姿态
    if (currentModel) {
        if (objectPoses && objectPoses[currentFrameIndex]) {
            // 获取当前帧的变换矩阵
            ...
            const numComps = Math.floor(flatPose.length / 4);

            if (numComps > 1) {
                // Multi-component: Reset parent transform
                currentModel.position.set(0, 0, 0);
                currentModel.quaternion.set(0, 0, 0, 1);
                currentModel.scale.set(1, 1, 1);

                for (let c = 0; c < numComps; c++) {
                    const compRows = flatPose.slice(c * 4, (c + 1) * 4);
                    const poseGL = new THREE.Matrix4().set(
                        compRows[0][0], compRows[0][1], compRows[0][2], compRows[0][3] * global_scale,
                        compRows[1][0], compRows[1][1], compRows[1][2], compRows[1][3] * global_scale,
                        compRows[2][0], compRows[2][1], compRows[2][2], compRows[2][3] * global_scale,
                        compRows[3][0], compRows[3][1], compRows[3][2], compRows[3][3]
                    );
                    ...
                    if (currentModel.children && currentModel.children[c]) {
                        const child = currentModel.children[c];
                        child.position.copy(position);
                        child.quaternion.copy(quaternion);
                        child.scale.copy(scale).multiplyScalar(global_scale);
                        child.visible = true;
                    }
                }
            } else {
                // Single component: Standard code
                const poseGL = new THREE.Matrix4().set(
                ...
                currentModel.scale.copy(scale).multiplyScalar(global_scale);
                currentModel.position.copy(position);
                currentModel.quaternion.copy(quaternion);

                // Reset children transformations in case previous sequence had multiple components
...
async function loadSingleFrameData(index, parentDir, loadToken) {
    ...
    try {
        ...
        // 1. Load hand GLB for this frame
        const tex = null;
        const handMesh = await loadGLBWithSignal(handMeshPath, controller.signal, null, 1.0, individualGroup, `hand_${index}`).catch((err) => {
        ...
        if (handMesh) {
            ...
            // Calculate hand centroid on frame 0 to set the camera rotation target center
            if (index === 0) {
                individualGroup.updateMatrixWorld(true);
                const box = new THREE.Box3().setFromObject(individualGroup);
                console.log(`[DEBUG] Frame 0 load: box empty? ${box.isEmpty()}, min: ${JSON.stringify(box.min)}, max: ${JSON.stringify(box.max)}`);
                if (!box.isEmpty()) {
                    const center = new THREE.Vector3();
                    box.getCenter(center);
                    handCentroid = center.clone();
                    console.log(`[DEBUG] Frame 0 load: calculated handCentroid = (${handCentroid.x}, ${handCentroid.y}, ${handCentroid.z})`);
                    controls.target.copy(handCentroid);
                    controls.update();
                    console.log(`[DEBUG] Frame 0 load: updated controls.target = (${controls.target.x}, ${controls.target.y}, ${controls.target.z})`);
                } else {
                    console.warn(`[DEBUG] Frame 0 load: box is empty!`);
                }
            }
...
export function initDemoViewer({ containerId = 'viewer', galleryId = 'thumbnailGallery', thumbnailList = [] } = {}) {
    ...
    // Attach headlight to camera so it shines in camera direction
...
async function loadGLB(glbPath, transformMatrix = null, scale = 1.0, group = null, entityName = null, loadToken = null, color = 0xa8e6cf) {
...
async function loadHOIDataFromMetadata(metadata, parentDir, loadToken = null) {
    try {
        ...
        try {
            loadedModel = await loadGLB(object_mesh_path, null, 1.0, null, null, loadToken);
        } catch (err) {
            console.warn(`Object model not found or failed to load: ${object_mesh_path}. Continuing without object mesh.`);
        }
        ...
        if (res.ok) {
            ...
            if (objectPoses && objectPoses.length > 0) {
                ...
                controls.target.set(0, 0, initObjectPosition.z);
                controls.update();
                console.log(`[DEBUG] loadHOIDataFromMetadata object poses set: controls.target = (${controls.target.x}, ${controls.target.y}, ${controls.target.z})`);
            }
        } else {
            console.warn(`Object poses file not found or failed to load: ${object_poses_path}`);
            objectPoses = null;
            // Default camera target if no object pose exists
            initObjectPosition = new THREE.Vector3(0, 0, -0.3 * global_scale);
            controls.target.set(0, 0, initObjectPosition.z);
            controls.update();
            console.log(`[DEBUG] loadHOIDataFromMetadata default set: controls.target = (${controls.target.x}, ${controls.target.y}, ${controls.target.z})`);
        }
...
function animate() {
    ...
    // 更新物体姿态（应用坐标系转换和缩放）
    const totalFrames = (objectPoses && objectPoses.length > 0) ? objectPoses.length : (currentMetadata ? currentMetadata.num_frames : 0);
    if (!isAnimationPaused && totalFrames > 0 && handMeshes.length > 0 && videoFrames.length > 0 && cameraFrustums.length > 0) {
        ...
        if (currentTime - lastPoseUpdateTime >= poseUpdateInterval) {
            ...
            const frameIndexValue = document.getElementById('frameIndexValue');
            if (frameIndexSlider && frameIndexValue) {
                const progress = (currentFrameIndex / (totalFrames - 1)) * 100;
        ...
        thumbnailDiv.onclick = async () => {
            ...
            if (currentMetadataPath) {
                const stashedData = {
                    ...
                    handCentroid: controls.target.clone(),
                    cameraParams: {
                        fov: camera.fov,
            ...
            controls.object.up.set(0, 1, 0);
            controls.update();
            console.log(`[DEBUG] thumbnailDiv.onclick: Reset camera, controls.target = (${controls.target.x}, ${controls.target.y}, ${controls.target.z})`);
            ...
            if (restoredData) {
                ...
                handCentroid = restoredData.handCentroid ? restoredData.handCentroid.clone() : null;
                console.log(`[DEBUG] Cache hit restore: handCentroid = ${handCentroid ? `(${handCentroid.x}, ${handCentroid.y}, ${handCentroid.z})` : 'null'}`);
                if (handCentroid) {
                    controls.target.copy(handCentroid);
                    controls.object.up.set(0, 1, 0);
                    controls.update();
                    console.log(`[DEBUG] Cache hit restore: updated controls.target = (${controls.target.x}, ${controls.target.y}, ${controls.target.z})`);
                } else if (initObjectPosition) {
                    controls.target.set(0, 0, initObjectPosition.z);
                    controls.object.up.set(0, 1, 0);
                    controls.update();
                    console.log(`[DEBUG] Cache hit restore fallback: updated controls.target = (${controls.target.x}, ${controls.target.y}, ${controls.target.z})`);
                }
...
// Debug hook to allow console inspection
window.__debug__ = {
    getScene: () => scene,
    getCamera: () => camera,
    getControls: () => controls,
    getCurrentModel: () => currentModel,
    getCurrentHandMesh: () => currentHandMesh,
    getHandMeshes: () => handMeshes,
    getObjectPoses: () => objectPoses,
    getCurrentFrameIndex: () => currentFrameIndex,
    getInitObjectPosition: () => initObjectPosition,
    getGlobalScale: () => typeof global_scale !== 'undefined' ? global_scale : 4.0
};
